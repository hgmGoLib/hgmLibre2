...
//
// 🔴 拿到游程之后【展不展开】要看这条 pattern 有没有长度上界 —— 这决定了 ResolveSpan 调几次:
//
//	有上界 (AAA-[A-Za-z0-9]{8,16} 这种): 展开。游程里每个端点确实是一个【不同的】匹配,
//	  而且游程最长也就上界减下界那么长, 逐个解析的代价可以忽略。
//	没上界 ({n,} · * · + · .*): 【不要展开】, 取最外那一个端点解析一次就够
//	  (正向 set 取 Hi = 最长, 反向 set 取 Lo = 最长)。这种 pattern 的一整条游程往往就是
//	  【同一个匹配】在每个可能的切点上各报了一次 —— 展开得到的是一串【互相嵌套】的区间
//	  ([0,4) [0,5) … [0,65536)), 没有新信息, 下游"相交即丢"又会把它们全折回一个。
//	  实测: [a-z]{4,} 撞上 64KiB 全小写正文 = 1 条游程 / 1 个匹配 [0,65536), 却有 65533 个
//	  右端; 全展开就是 65533 次解析、每次都从自己那个右端一路走回偏移 0 = O(正文²)。
//
