...
class DFA {
 public:
  ...
  // ── per-pattern 存活位: 用几个 64 位字 (Re2SetFrl 的分量切分靠它) ──
...
// ── per-pattern 存活位 ──────────────────────────────────────────────────────
// 目的: 给 Re2SetFrl 一个【分量边界】—— 某条 pattern 在某个位置上没有活线程,
...
// 🔴 go build 的缓存【不看 .inc 文件】—— 只改 re2_dfa_spanscan.inc 不会触发重编,
//    跑出来的还是上一版二进制 (踩过一次: 明明加了 fprintf 却一行都不打)。
//    改完 .inc 之后把下面这个数字 +1, 让本文件内容变一下, 才会真的重编。
//    (换 CGO_CXXFLAGS 也行 —— 那是另一个缓存键, 所以 -D 开关的对照组不受影响。)
#define HGM_SPANSCAN_INC_CACHEBUST 5
