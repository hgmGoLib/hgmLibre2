...
type re2setScanFn func(body string, req *Re2Set_req_t) error

// scanFlat 跑一遍, 把区间按 pattern 归拢成扁平 (start,end) 表, 同时把命中位表带回来。
func scanFlat(t *testing.T, scan re2setScanFn, text string) (map[int32][]int32, []int32) {
	t.Helper()
	out := map[int32][]int32{}
	var hits []int32
	err := scan(text, &Re2Set_req_t{
		...
		HitIndexResultFn: func(h []int32) bool {
			hits = append(hits, h...)
			return true
		},
	})
...
// newFll / newRrl / newFrel 开一个工作区 (连同它那张表), 测试结束自动 Close。
