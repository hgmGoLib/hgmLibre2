...
// frlScan 是"开一个 Re2SetFrl, 扫一遍, 把区间收成一个数组"的测试用简写。
// 🔴 生产路径别这么写 (那块 dst 是 ∝ 命中数的 ratchet 缓冲, 正是分批接口要躲开的东西),
// 这里是判据代码, 语料才一百多字节。
func frlScan(t *testing.T, pats []Re2SetFrlPattern_t, text string, batch int) []SetMatch {
	t.Helper()
	s, err := NewRe2SetFrlMaxMem(pats, 64<<20)
	if err != nil {
		t.Fatalf("建 Re2SetFrl 失败 pats=%v: %v", pats, err)
	}
	defer s.Close()
	var got []SetMatch
	err = s.Scan(text, NewRe2SetFrlBuf(batch), func(idx, st, en []int32) bool {
		for i := range idx {
			got = append(got, SetMatch{Index: idx[i], Lo: st[i], Hi: en[i]})
		}
		return true
	})
	if err != nil {
...
// frlBrute 是穷举出来的【最右终点最长】不重叠序列 —— 与本库无关, 只用 stdlib。
// 上界从正文末尾起, 反复取"终点最靠右且 <= 上界"的匹配, 同终点取最长 (= 起点最靠左),
// 收下之后把上界压到它的起点。返回升序。
//
// 🔴 这不是 msrBrute (反向 MatchScanner 那个)。那个先挑【起点】最靠右的, 这个先挑
//    【终点】最靠右的 —— aa|a 撞 "aaa" 两者给出完全不同的答案, 见 TestRe2SetFrl_Shape。
func frlBrute(pat, text string) []SetMatch {
...
// TestRe2SetFrl_Shape 钉住对外说过的口径, 并且证明它与另外两个口径【真的不一样】——
// 否则下面那些对拍全是空转。
func TestRe2SetFrl_Shape(t *testing.T) {
	// aa|a 撞 "aaa": 三个口径三个答案。这一格就是整套判据的自检。
	got := frlScan(t, []Re2SetFrlPattern_t{{Pattern: `aa|a`}}, "aaa", 64)
	...
	got = frlScan(t, []Re2SetFrlPattern_t{{Pattern: `b|abc`}}, "abc", 64)
	if fmt.Sprint(got) != "[{0 0 3}]" {
		t.Fatalf("要 [{0 0 3}] 得到 %v", got)
	}

	// 定长: "12 345 6789" 里 \d{3} 的终点是 6 和 11, 从右往左各取一次。
	got = frlScan(t, []Re2SetFrlPattern_t{{Pattern: `\d{3}`}}, "12 345 6789", 64)
	if fmt.Sprint(got) != "[{0 3 6} {0 8 11}]" {
		t.Fatalf("定长档不对: 要 [{0 3 6} {0 8 11}] 得到 %v", got)
	}
}

// frlPats 是下面几个对拍共用的名单。前五条是 matchscan.go 头注里那几个"会把猜起点的路
...
// frlCorpus 按 pattern 自己的 AST 造一篇【真的撞得上】的语料 (随机字节撞不出真匹配 = 空转绿)。
// msrGen 在 matchscan_reverse_test.go 里, 两边共用。
func frlCorpus(ast *syntax.Regexp, rng *rand.Rand) string {
...
// TestRe2SetFrl_VsBrute 是这条路正确性的【全部】依靠。
// 它一并证明了"存活位切分量不改变答案"—— 判据 frlBrute 是【不分量】的全局穷举。
func TestRe2SetFrl_VsBrute(t *testing.T) {
	rng := rand.New(rand.NewSource(20260831))
	for _, pat := range frlPats {
		...
		for round := 0; round < 200; round++ {
			text := frlCorpus(ast, rng)
			got := frlScan(t, []Re2SetFrlPattern_t{{Pattern: pat}}, text, 64)
			want := frlBrute(pat, text)
			hits += len(want)
			if len(got) != len(want) {
...
// TestRe2SetFrl_MultiPattern: 整表一起扫, 逐条与"这一条自己扫"的判据对。
// 跨 pattern 一概不合并 —— 两条 pattern 在同一片正文上照样各给各的答案。
func TestRe2SetFrl_MultiPattern(t *testing.T) {
	pats := make([]Re2SetFrlPattern_t, len(frlPats))
	asts := make([]*syntax.Regexp, len(frlPats))
	for i, p := range frlPats {
	...
	for round := 0; round < 40; round++ {
		...
		for i := range asts { // 每条 pattern 都往里塞一点, 保证整表都撞得上
			sb.WriteString(frlCorpus(asts[i], rng))
		}
		text := sb.String()
		got := frlScan(t, pats, text, 64)
		byID := map[int32][]SetMatch{}
		for _, m := range got {
			byID[m.Index] = append(byID[m.Index], m)
		}
		for i, p := range frlPats {
			want := frlBrute(p, text)
			have := byID[int32(i)]
...
// TestRe2SetFrl_Batch: 批缓冲大小只决定过桥次数, 不许影响结果 —— 一次一条和一次四千条
// 必须逐处相同 (分量跨批续上的那段逻辑靠这个钉住)。
func TestRe2SetFrl_Batch(t *testing.T) {
	pats := make([]Re2SetFrlPattern_t, len(frlPats))
	asts := make([]*syntax.Regexp, len(frlPats))
	for i, p := range frlPats {
		pats[i] = Re2SetFrlPattern_t{Pattern: p}
	...
	for round := 0; round < 40; round++ {
		...
		text := sb.String()
		one := frlScan(t, pats, text, 1)
		big := frlScan(t, pats, text, 4096)
...
// TestRe2SetFrl_ExistOnly: 配了 ExistOnly 的那几条只上命中表, 不出区间;
// 命中表本身要与 RegexpSet.Match 逐条相同。
func TestRe2SetFrl_ExistOnly(t *testing.T) {
	pats := []Re2SetFrlPattern_t{
		{Pattern: `\d{3}-\d{2}-\d{4}`},
	...
	s, err := NewRe2SetFrlMaxMem(pats, 64<<20)
	if err != nil {
		t.Fatalf("建 Re2SetFrl 失败: %v", err)
	}
	defer s.Close()
	buf := NewRe2SetFrlBuf(16)
	texts := []string{
	...
	for _, text := range texts {
		var got []SetMatch
		if err := s.Scan(text, buf, func(idx, st, en []int32) bool {
			for i := range idx {
				got = append(got, SetMatch{Index: idx[i], Lo: st[i], Hi: en[i]})
			}
			return true
		...
		for i := range pats {
			if s.Hit(i) != want[int32(i)] {
				t.Fatalf("text=%q 第 %d 条命中表不对: Frl=%v set.Match=%v",
...
// TestRe2SetFrl_EmptyMatchRejected: 能匹配空串的条配区间档【建的时候】就报错, 与正文无关。
func TestRe2SetFrl_EmptyMatchRejected(t *testing.T) {
	if _, err := NewRe2SetFrl([]Re2SetFrlPattern_t{{Pattern: `a*`}}); err == nil {
		t.Fatal("a* 配区间档应当当场报错")
	}
	// 同一条配 ExistOnly 就行 —— 它只上命中表, 不进游程。
	s, err := NewRe2SetFrl([]Re2SetFrlPattern_t{{Pattern: `a*`, ExistOnly: true}})
	if err != nil {
		t.Fatalf("a* 配 ExistOnly 应当能建: %v", err)
	}
	defer s.Close()
	if err := s.Scan("bbb", NewRe2SetFrlBuf(8), func(i, a, b []int32) bool {
		t.Fatalf("ExistOnly 不该出区间: %v %v %v", i, a, b)
		return false
	}); err != nil {
		t.Fatalf("Scan: %v", err)
	}
...
// TestRe2SetFrl_EarlyStopAndReuse: 回调返 false 当场收工 (不算错), 而且同一个工作区
// 反复 Scan 不串味。
func TestRe2SetFrl_EarlyStopAndReuse(t *testing.T) {
	s, err := NewRe2SetFrlMaxMem([]Re2SetFrlPattern_t{{Pattern: `\d{3}`}}, 64<<20)
	if err != nil {
		t.Fatalf("建 Re2SetFrl 失败: %v", err)
	}
	defer s.Close()
	text := "111 222 333 444 555"
	buf := NewRe2SetFrlBuf(1)
	n := 0
	if err := s.Scan(text, buf, func(i, a, b []int32) bool { n += len(i); return n < 2 }); err != nil {
	...
	for round := 0; round < 3; round++ {
		got := frlScan(t, []Re2SetFrlPattern_t{{Pattern: `\d{3}`}}, text, 64)
		all := 0
		if err := s.Scan(text, NewRe2SetFrlBuf(64), func(i, a, b []int32) bool {
			all += len(i)
			return true
		}); err != nil {
...
// TestRe2SetFrl_Closed: Close 之后再用只报错, 不 panic。
func TestRe2SetFrl_Closed(t *testing.T) {
	s, err := NewRe2SetFrl([]Re2SetFrlPattern_t{{Pattern: `abc`}})
	if err != nil {
		t.Fatalf("建 Re2SetFrl 失败: %v", err)
	}
	s.Close()
	s.Close() // 幂等
	if err := s.Scan("abc", NewRe2SetFrlBuf(4), nil); err == nil {
