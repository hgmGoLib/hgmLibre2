...
* 非 stdlib 的额外方法:
    ...
    - RegexpSet.NewMatchScanner + (*MatchScanner).SetWanted / Scan / HitIDs / Hit
      ...
      FindAllStringIndex —— 命中 k 条就是 1+k 遍全文。
        ...
        // unres 里的那几条这次拿不到 (能匹配空串 / 反向编不出来 / 游程乱序 / DFA 预算不够)
        //   → 把本遍已经收到的这几条【全丢掉】, 照老路 re.FindAllStringIndex
        //   (作废可能发生在已经交出去几处之后; 没进 SetWanted 的【不】算作废, 一处都不会来)
      ...
      🔴 这一层【一个字节都不留】: 游标推进在回调里当场跑完, 收口出来的区间写进一块固定的
         12KB 缓冲 (1024 处), 满了就交出去、就地复用。底下 native 那层本来就是 sqlite3_step
         式的 (一批 4096 条 = 48KB, 装满就挂起回调, 缓冲循环复用, 不随正文长度涨); 要是这一层
         把结果 append 起来等扫完再还给调用方, 那个固定缓冲就白设计了 —— 真表上游程约
         30741 条/MB (0.23MB/MB), 收口后的输出还有 0.037MB/MB, 200MB 的 body 就是每个并发
         扫描 7.4MB 常驻, 再来个"照抄一份给调用方"的 AppendMatches 就是两份。
         (归拢成"按 pattern 一张表"也是攒 —— 所以那一步交给调用方, 库这边只管一批一批交。)
         能这么做是因为同一条 pattern 的游程【跨批次也是升序】(正向扫本来就从左往右走; 真表
         196744 条游程 / 50 批, 乱序 0 处)。万一乱序, 那一条当场作废进 unresolved 退回老路。
      🔴 SetWanted 不是可选优化, 也不能靠"调用方在回调里自己过滤"顶替: 挡掉的是那几条的
         【左端回推】(这一层真花钱的那步), 不只是少交几处。门上很多位只当外层短路的 bool 用
      ...
      🔴 和 FindAllStringIndex 的关系【分两档, 只有一档敢说一样】:
         定长档 (min==max): 逐字节相同, 可论证 —— 右端 e 的起点只能是 e-min (唯一), 长度也唯一,
           剩下的"从左往右贪心取不相交"正是 FindAll 的规则。钉在 TestMatchScanStrictVsFindAll
           + 6 万条随机定长 pattern 对拍。
         变长档 (min<max): 【不保证】和 FindAll 挑的是同一处。两个口子, 都实测有反例:
           取最长 ≠ 贪心先撞上 —— 贪心量词是按优先序回溯不是全局最长。x{1,3}[a-c]?(?:ab|cd)?
             撞 "xab": FindAll 给 [0,2)="xa" (让 [a-c]? 先吃), 这里给 [0,3)="xab"。
           起点未必随右端单调 —— (?:ab)?[bc]{1,2} 撞 "axbabbyxx": 这里给 [4,6)="bb",
             FindAll 给 [3,6)="abb"。更靠右的右端配了更靠左的起点, 游标推过去就回不来。
      🔴 这两个口子上给的段【是真匹配但不是 FindAll 那一处】。消费点要是拿这一段去过校验位
         (身份证 · IBAN mod-97 · Luhn) 或者做"段里有没有空格"这类判断, 边界差一字节就是
         校验失败 = 【无声漏报】, 比"没检测到"更难查。这种调用方只能挑【定长档】用。
         曾经为此加过一个 SetExactOnly 开关 (把变长档整个关掉), 2026-08-24 删了 ——
         它买不到性能: 真表 6.4MB 上全开 1.82×, 只留定长档 1.18×, 等于把三分之二的收益
         交出去换一个"仍然要调用方自己判断哪条能用"的开关。
      🔴 但这件事【两头 \b 一锚就没了】: word boundary 把起点钉死, 回看窗口里合法起点只剩
         一个, "挑哪个"根本不发生。量过 (各 12 万处对账): 裸 pattern 与 FindAll 不同 60 处
         (少盖 30 字节 · 多盖 17 字节 · 处数变了 16 处), 换成 \b(?:…)\b 之后【0 处】。
         身份证 / 护照号 / NRIC 那类刚性格式本来就都写成 \b 锚住的, 所以真表 6.4MB 上
         31379 处逐处全等 —— 是这个原因, 不是运气。反过来说: 变长条要是【没有】两头锚点,
         位置就只当"这附近有东西"用, 别切片再判。
      ...
      实测 (真实 155 条规则表 × 6.4MB 真语料 · 命中 47 条 · 稳态 · 生产预算 64MB):
        ...
        🔴 中间有一版装着 PatternLeftmostLongestSafe 静态闸, 它挡下 12 条, 那 12 条照样对
           ...
           回到 15.03×。剩下唯一接不了的是 25 条【没有长度上限】的 (邮箱那种), 没有 maxL
