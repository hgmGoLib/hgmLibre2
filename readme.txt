...
* 非 stdlib 的额外方法:
    ...
    - RegexpSet.NewMatchScanner + (*MatchScanner).SetWanted / Scan / HitIDs / Hit / AppendMatches
      ...
      🔴 【只在同一条 pattern 内部去重, 跨 pattern 一概不合并】: 两条 pattern 撞在同一片正文上
         ...
         空格"分流; 合了就是漏检)。这条在真表上量过 (doc/plan12/20260824_208re2set.txt 十):
         6.4MB 真语料 74249 处命中里, 被 ≥2 条 pattern 盖住的字节占已盖住字节的 55.6%,
         同一字节最多被 8 条盖; 而"哪一条该赢"要等消费点把校验位跑完才知道 ——
         "S1234567X" 上 NRIC 那条会被 ValidateNRIC 毙掉, 库要是先按位置把另一条合并没了,
         这一段就一条都不出 = 无声漏报。跨条合并是调用方 (engine/sd_multi_hit.go 的 prio) 的事。
