...
* 非 stdlib 的额外方法:
    ...
    - 性能 (spanscan_bench_test.go · 64KiB 正文 · 10 条通用 pattern · Ryzen 5900X · 稳态复用):
      ...
      FindAllStringIndex (命中 k 条 = 1+k 遍全文扫描, 而且后面那 k 遍是最贵的非锚定扫描)。
        ...
        旧实现               92.0us      411.6us      577.6us  给 4 处
        只扫不解析           89.9us       88.7us      229.2us
        扫 + 每游程解析一次  89.9us       91.4us     1060.5us  给 9 处
        扫 + 每端点解析一次  91.3us      100.8us      5592ms   给 65541 处
        同上 + bound=64      92.9us      101.1us       14.6ms  给 65541 处
      三档全部 0 B/op 0 allocs/op (工作区复用 + 两处出参都走按值返回的孪生, 见 VENDOR.txt)。
      怎么读:
      · 0 命中 —— 产线绝大多数正文长这样。两边都是一遍扫, 打平 (±3% 噪声内)。
      · 稀疏命中 —— 4.5 倍。省掉的正是旧实现那 1+k 遍全文重扫。
      · 最坏输入 —— 语料是【全小写无空格】, 表里 [a-z]{4,} 从偏移 4 起每一个位置都是端点。
        这一档必须分开看, 不能拿总时间对比:
          · "每游程解析一次" 与 FindAllStringIndex 同口径 (每处给一个匹配)。不掐 bound 是
            1.8 倍【慢】—— 9 次解析每次都一路走到正文开头 ([a-z]{4,} 真能无限延伸);
            掐 bound=64 就是 229.7us = 2.5 倍快。这一档就是 bound 存在的理由: 给这类
            "能无限延伸"的 pattern 配一个回看上限, 解析成本从 O(正文) 钉回常数。
          · "每端点解析一次" 是【调用方把游程展开】之后的成本, 不是库这一层的成本。
            这一档的 65541 个右端里有 65533 个来自【同一条游程、同一个匹配】([a-z]{4,}
            吃掉整篇, 旧实现同样只给 1 处 [0,65536)) —— 展开得到的是 65533 个【互相嵌套】
            的区间, 没有新信息, 而每一个都要从自己那个右端一路走回偏移 0, 于是 O(正文²)。
            所以结论不是"这条路慢", 是【没上界的 pattern 不该展开游程】: 取最外那个端点
            解析一次就够。有上界的 pattern (…{8,16}) 该展开也展得起 —— 游程就那么长,
            稀疏语料上全展开也只比不展开多 9us (100.8 vs 91.4)。判据写在 SetSpan 的注释上。
