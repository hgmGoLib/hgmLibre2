...
一遍扫一张表要"命中在哪" 先看 doc/三种去重叠模式.md —— 三个类型 (Re2Set_fll_t /
Re2Set_frel_t / Re2Set_rrl_t) 的口径差在哪、同一个输入三种答案、实测价钱、怎么选、怎么用、
怎么量。🔴 leftmost-longest 只是三种去重叠口径之一, 既不是最快的也不一定是你要的那个。
其中 fll 那条保证的长版 (凭什么敢无条件说 leftmost-longest、对拍要拿哪个当 oracle,
以及 §8 反向那一侧的 rightmost-longest) 看 doc/fll的leftmost-longest保证.md。
...
* 非 stdlib 的额外方法:
    ...
    - RegexpSet.NewRe2Set_fll + (*Re2Set_fll_t).Scan / GetPatternLen / GetViableOneStats / Close
      ...
      ── 两层, 分得死死的 (2026-09-01) ─────────────────────────────────────────
        ...
        Re2Set_*_t      策略对象: set 引用 + mode + 长度区间表 +【补端点用的
                        单条正向/反向对象缓存】(惰性建, 最大那张生产规则表 9.6MB)  进程级 · 并发安全
      ...
      怎么做到的 (详见 doc/fll的leftmost-longest保证.md):
        ...
        变长 (min < max)   【两步】:
                           ...
                           为什么这就是 leftmost-longest (两条, 都要):
                             ① 候选集不漏: 真答案 [s,E) 若 s ∈ [游标,e), 则 E >= e (e 是
                                >游标 的最小右端) ⟹ text[s,e) 是可行前缀 ⟹ s 必在候选里。
                                升序试 ⟹ 第一个通过的必然是 leftmost; fwd 是 longest 口径编的,
                                锚定在 s 上给的就是最长右端 ⟹ 严格 leftmost-longest。
      ...
      ── 🔴 Re2Set_stats_t: "试/看"是这条路唯一要盯的那个数 ───────────────────────
        ...
        都没验。2026-08-28 实测 11 份 100MB 真语料 × 9 张生产真门 = 99 格【全是 1.00】。
        ...
        见 TestMatchScanPerfHard。真表形状上不发生。
      ...
      那时候补端点是"默认档 + 一个旋钮 + 一个独立类型"三条并存:
        路 A (spanFast)      反向【只种 accept】回推一个起点 + 正向锚定取最长右端。两趟,
                             回看窗口【相交】。给的是【第三种口径】—— 既不是 leftmost-first
      ...
      原始报表在 doc/补起点换路的实测账_20260828.txt):
        ...
        价钱  11 条门链合计, D2 【每一条都是最快的】: 相对路 A 0.48~0.91x, 相对路 B 0.6~1.0x。
              "试/看"在 99 格里全是 1.00。
        ...
        Scan 的 err                      这一遍不算数 (已经交出去的批次也不算), 整篇走老路。
                                         三种来由都是 maxMem 配小了:
                                           ...
                                           · 补端点要的那个单条对象编不出来 (正向单条 /
                                             反向单条 set, 两个都可能);
                                           · 反向回推那一趟 DFA 放弃。
                                         🔴 最后这条【只有反向那一趟会碰到】: 正向锚定走的是
                                            单条 RE2::Match, DFA 放弃还能退到 OnePass/BitState/
                                            NFA。反向回推 RE2 自己也只有 DFA 一条路 (它内部求
                                            匹配左端就是这么干的)。
                                         🔴 "反向单条 set 编不出来"这一条【不兜底】: 2026-08-28
                                            逐字节量过 —— 590 条生产 pattern 里"反向比正向贵"
                                            的只有 16 条 (全是 {n,} 开放尾巴), 最大倍率 1.021x。
                                            要让"正向 set 编得出来而反向单条编不出来"真的发生,
                                            set 里得只装【一条】pattern 且 maxMem 恰好卡进那条
                                            阈值往上 2% 的带子里 (实测那条 JWT 三段式: 正向
                                            3580 字节 / 反向 3654 字节, 窗口 74 字节)。多条的表
                                            上正向 set 本身就贵出几个量级, 这个窗口从结构上不
                                            存在。能走到这里只说明 maxMem 配错了 —— 那就该报错
                                            让人调大, 静默换一条实现只会把配置错误藏起来。
      ...
      🔴 下面两组是【2026-08-28 换路之前】量的, 那时候还有路 A / 路 B 之分; 换路之后的数
         在上面"老账"那一节和 doc/补起点换路的实测账_20260828.txt。这两组留着是因为它们
         回答的是另一个问题:【接这一层比不接强多少】(相对老的"门 Match + 逐条 FindAll"),
         那个倍数换路之后只会更大。
      🔴 两组【表和语料不同, 不许横着比】。
    ...
    - RegexpSetReverse.NewRe2Set_rrl + (*Re2Set_rrl_t).Scan / GetPatternLen / Close:
      ...
      ── 为什么反向【更好】做, 不是更难做 ──────────────────────────────────────
      正向那一遍 DFA 交出来的是匹配【右端】, 起点得在 Go 这侧【回推】(上面"变长怎么补"
      那一段: 反向收候选 + 升序逐个正向锚定验)。反向交出来的是【左端】= 起点, 而 leftmost/
    ...
    - RegexpSet.NewRe2Set_frel + (*Re2Set_frel_t).Scan / GetPatternLen / Close
      ...
      8 个布局各取最小; 尺子在库外那棵树上, 这里只登记结论):
        · 12 格里 11 格 Frel 更快, 端到端 1.06~2.24 倍 d2; 只看补起点那一层 2.2~2.4 倍。
