...
* 非 stdlib 的额外方法:
    ...
    - RegexpSet.NewMatchScanner + (*MatchScanner).SetWanted / Scan / HitIDs / Hit / AppendMatches
      ...
      FindAllStringIndex —— 命中 k 条就是 1+k 遍全文。
        ...
        // ok==false: 这一条拿不到 (不在 wanted / 开了 ExactOnly 的变长条 / 能匹配空串 /
        //            口径不安全 / 反向编不出来 / 游程乱序 / DFA 预算不够)
        //            → 照老路 re.FindAllStringIndex
        ms.SetExactOnly(true)               // 只要"逐字节等于 FindAll"的那一档, 见下面红字
      ...
      🔴 这两个口子上给的段【是真匹配但不是 FindAll 那一处】。消费点要是拿这一段去过校验位
         ...
         校验失败 = 【无声漏报】, 比"没检测到"更难查。这种调用方必须 SetExactOnly(true)。
         代价是真的: 真表 6.4MB 上全开 13.9×, 只留定长档 1.19× —— 收益大头在变长档,
      ...
      实测 (真实 155 条门表 × 6.4MB 真前端产物 · 命中 47 条 · 稳态 · 生产预算 64MB):
        ...
        同上但 SetExactOnly(true) (87 条要位置的里只有 28 条是定长): 371.1ms → 312.8ms = 1.19×
