// findallindex.go —— FindAllIndex: 一遍扫正文, 边扫边把"哪条 pattern 的匹配端点落在哪一段"
// 交给回调。这是本库多正则一侧的【底座】, 上面那层"算出完整区间"(matchscan.go) 就搭在它身上。
//
// ── 它替掉的是哪段路 ────────────────────────────────────────────────────────
...
//
// ── 吐的是【端点游程】, 不是匹配区间 ────────────────────────────────────────
// 回调拿到的是 (ReIndex, lo, hi): 第 ReIndex 条 pattern 的匹配端点落在 lo..hi 里的
// 【每一个】值上 (原文字节偏移,【两端都含】, 恒 lo <= hi)。
//
//	正向 RegexpSet        : 端点 = 匹配【右端】(不含), 即 text[?:endHi] 是一个匹配
//	反向 RegexpSetReverse : 端点 = 匹配【左端】(含),   即匹配从 text[startLo] 开始
//
...
//    最容易错的地方。名字里也没有 start/end 的暗示 —— 它俩就叫 endLo/endHi (正向) 和
//    startLo/startHi (反向), 一眼能看出说的是哪一端。
//
...
//
// ── 为什么给回调而不是给一个数组 ────────────────────────────────────────────
// 游程条数没有上界 (真表实测约 30741 条/MB, 200MB 的 body 就是 47MB), 攒成数组等于让内存
// 跟着正文长度走。回调让调用方【当场消费完】, 库这边只留一个固定大小的缓冲。
// 代价是每条游程一次间接调用 —— 6.4MB 上 19.7 万次, 约 0.4~1.0ms, 占总时长 2~4%。
//
// ── alloc 是什么 ────────────────────────────────────────────────────────────
...
//
// ── 偏移为什么是 int32 ──────────────────────────────────────────────────────
// ① 宽度锁死, 32 位/64 位平台上一样宽; native 侧本来就是 int32, 零转换。
...
type RegexpSet_FindAllIndex_Alloc_t struct {
	h   *C.cre2_spanscan
	set *RegexpSet
	// buf 是一批游程的落地处, 紧挨着的 int32 三元组 (Index, Lo, Hi) —— native 直接往这块内存
	// 写, 不做逐条转换。长度恒为 batch*3。
	buf []int32
...
func newFindAllIndexAlloc(s *RegexpSet, batch int) (*RegexpSet_FindAllIndex_Alloc_t, error) {
	...
	a := &RegexpSet_FindAllIndex_Alloc_t{h: h, set: s, buf: make([]int32, batch*3)}
...
// FindAllIndex 扫 text 一遍, 每收口一段端点游程就调一次 matchFn:
// 第 ReIndex 条 pattern 的匹配【右端】(不含) 落在 endLo..endHi 里的每一个值上 (两端都含)。
//
// alloc 传 nil = 当场建一个用完就扔 (多一笔 native 分配); 热路径上传一个长期复用的。
// 没有任何命中时 matchFn 一次都不会被调。
//
...
func (s *RegexpSet) FindAllIndex(text string, alloc *RegexpSet_FindAllIndex_Alloc_t,
	matchFn func(ReIndex int32, endLo int32, endHi int32)) error {
	return findAllIndex(s, text, alloc, matchFn)
}
...
func (s *RegexpSet) FindAllIndexBytes(text []byte, alloc *RegexpSet_FindAllIndex_Alloc_t,
	matchFn func(ReIndex int32, endLo int32, endHi int32)) error {
	err := findAllIndex(s, bytesStr(text), alloc, matchFn)
	runtime.KeepAlive(text)
	return err
}

// FindAllIndex 扫 text 一遍 (从末尾往前走【原始 buffer】, 不反转正文, 不复制正文),
// 每收口一段端点游程就调一次 matchFn: 第 ReIndex 条 pattern 的匹配【左端】(含) 落在
// startLo..startHi 里的每一个值上 (两端都含)。
//
...
//
// 其余 (alloc 语义 · 闭区间 · 顺序 · error) 与正向那个完全一致。
func (r *RegexpSetReverse) FindAllIndex(text string, alloc *RegexpSet_FindAllIndex_Alloc_t,
	matchFn func(ReIndex int32, startLo int32, startHi int32)) error {
	return findAllIndex(r.s, text, alloc, matchFn)
}

// FindAllIndexBytes 同 FindAllIndex, 但正文是 []byte (零拷贝)。
func (r *RegexpSetReverse) FindAllIndexBytes(text []byte, alloc *RegexpSet_FindAllIndex_Alloc_t,
	matchFn func(ReIndex int32, startLo int32, startHi int32)) error {
	err := findAllIndex(r.s, bytesStr(text), alloc, matchFn)
	runtime.KeepAlive(text)
	return err
}
...
func findAllIndex(s *RegexpSet, text string, alloc *RegexpSet_FindAllIndex_Alloc_t,
	matchFn func(ReIndex int32, lo int32, hi int32)) error {
	if s.size == 0 {
		return nil
	}
	if len(text) > maxCInt {
	...
	for {
		...
		if n*3 > len(alloc.buf) {
			n = len(alloc.buf) / 3 // 防御: C 侧最多写 outcap/3 条
		}
		for k := 0; k < n; k++ {
			matchFn(alloc.buf[k*3], alloc.buf[k*3+1], alloc.buf[k*3+2])
		}
		if alloc.more == 0 {
			return nil
		}
