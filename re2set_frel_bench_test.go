...
//
// 三条路:
//   旧实现        set.Match 当快拒门 + 命中的每条各对整篇正文跑一遍 FindAllStringIndex
//                 (命中 k 条 = 1+k 遍全文, 后面那 k 遍是最贵的非锚定扫描)
//   fll           一遍正向 set 扫 + 逐个右端回推起点, 口径 leftmost-longest
//   frel          一遍正向 set 扫 + 存活位切分量 + 分量内一次结算, 口径最右终点最长
//
// ⚠ 口径不同 (见 re2set_frel.go 头注), 这里比的是【同一个问题的三种答法要多少钱】, 不是
//    "谁给的答案对"。答案对不对由 re2set_frel_test.go 的穷举判据管。
// ⚠ Frel 自己建自己那份正向 set, 没法与另外两条共用对象 —— 按 README.md "底座那一层的性能
//    对照" 一节末尾那条警告, 同一段代码换个 set 对象能差 5~8%。
//
...
//     每个变体【各取最小值】再比
// (doc/set性能优化经验.md 早写了这台机 ±20%~2倍的抖动, 这是目前见过最狠的一例。)
//
// ── 按上面那个法子量出来的 (64KiB · 10 条 pattern · 8 个布局取最小 · us) ──────
//     语料            旧实现   Re2SetFll      Re2SetFrel
//     zero 0 命中      97.5       98.5          97.2      三条打平
//     few  39 处       430.9      104.8         104.3     对旧实现 4.13x
//     most 最坏输入    598.9      478.7         575.6     对 Re2SetFll 0.83x
// 怎么读:
...
//  · few —— 4.13 倍。省掉的正是旧实现那 1+k 遍全文重扫。与 fll 打平, 但 Frel
...
//    的命中 (G2Note 要跑)。这一档 Frel 比 fll 慢 17%, 比旧实现快 4%。
//    真语料不长这样; 要在这种输入上也快, 只能让表少造端点 (那是表的问题, 不是这一层的)。
//
// ── 🔴 这三档语料只有 10 条手造 pattern, 【会低估】: 换成真实规则表是 1.4~2.2 倍 ──────
// 上面 few/most 那两行"与 fll 打平 / 慢 17%"是这份小语料的性质, 不是这一层的
// 性质。同样的两条路放到三张真实规则表上 (从一个调用方产品的源码里静态收出来的 368 条
// 字面量, 按形状分成 cred 64 条 / prompt 31 条 / body 160 条, 256KiB 正文 × 四档命中
// 密度, 8 个布局各取最小), 差距就出来了 —— 尺子在库外那棵树上, 这里只登记结论:
//
...
//    连填充词都能命中。这就是产线大表的常态。
//  · 与这条路的原型口径一致 (原型上量到端到端 1.3~1.9 倍 fll, 补起点层 1.4~2.2 倍);
//    落成产品版之后量出来只多不少。
